     ...
     : IRequestHandler<CreateRoomCommand, RoomDto>
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            var roomClass = await _roomClassRepository
                .GetByIdAsync(dto.RoomClassId, cancellationToken);

            if (roomClass is null)
                throw new NotFoundException(nameof(RoomClass), dto.RoomClassId);

            var exists = await _roomRepository
                .ExistsNumberInRoomClassAsync(dto.RoomClassId, dto.Number, cancellationToken);
